---
title: 'Text mining, sentiment analysis, and visualization Nikolaj202206222'
date: '21 marts 2025'
...
### Get the e-book 'A Game of Thrones':
```{r get-text}
...
Example: Just want to get text from a single page (e.g. Page 260)? 
```{r single-page}
got_p260 <- got_text[260]

```

See how that compares to the text in the PDF on Page 260. What has pdftools added and where?
# pdftools har tilføjet \n hvor der sker linjeskift i den oprindelige tekst.
...
# Min got_afinn2_n indeholder 201 observationer, så grafikken bliver lidt overvældende.
# Jeg tror ikke jeg kan afgøre, om brugen af ordet honor er enten positiv eller negativ i Game of Thrones. Jeg har ikke læst bogen, men vil gætte på, at det er overvejende positivt.
```

Look back at the Game of Thrones, and search for "honor" Is it typically associated with emotion, or something else? 
...
We can use the NRC lexicon to start "binning" text by the feelings they're typically associated with. As above, we'll use inner_join() to combine the Game of Thrones non-stopword text with the nrc lexicon: 
...
ggsave(plot = got_nrc_gg, 
       here("figures","got_nrc_sentiment.jpg"), 
       height = 12, 
       width = 8)

```

Wait, so "lord" is showing up in NRC lexicon as "negative"? Let's check:
```{r nrc-lord}
